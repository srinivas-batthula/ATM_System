...
def checkexist():                               #To check the existence of the file(s-bank.json)
    f='s-bank.json'
    if os.path.exists(f):
        fp=open(f, 'r')
        global d
...
class bank:
    ...
    def createacc():            # To create a new Account
        times=dt.datetime.now()
        lr=0
        age=0
        print()
        na=input("Enter your name : ").upper()
        ...
        clearsc_lines(1)
        sacno=str(acc)
        s = {               # Schema of customer JSON data.
            'name':na,
            'age':age,
            'bal':0.0,
            'transhistory':[]
    ...
    def test(self):                   # To check & validate the customer
...
class customer:
    ...
    def trans(c, amou, times):         # To update the transaction history in JSON-file
        s=c+str(amou)+" *//* "+str(times)
        global d
        global acno
        r=list(d[str(acno)]['transhistory'])
        r.append(s)
...
def clearsc_lines(n):                   # To clear the given n.o of lines on console
    ti.sleep(0.65)
    for _ in range(n):
...
def load():                 # Loader for all positions
    for i in range(8):
...
def clear_console():            # To clear the whole console
    ti.sleep(0.5)
...
def head(g):                # Main-program execution starter
    if g==1:
        bank.createacc()
    else:
        x=int(input("Enter your (4-digit) Account Number : "))
        clearsc_lines(1)
        cus1=bank(x)
        cus1.test()


def hor():                  # S-letter animation-methods
        print(" * "*(6))
        ti.sleep(0.2)
def ver(n):
    for i in range(3):
...
def start1():               #1 Execution start line
    checkexist()
    hor()
    ver(0)              # ' S ' animation
    hor()
...
def start2():                       #2 Start - End Project Runner (Main)method of whole program
    g=int(input("1. Create new account.\n2. LogIn to existing account.\nEnter your choice : "))
    clearsc_lines(3)
    head(g)
